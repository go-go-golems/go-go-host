...
This playbook explains how to design and implement new `go-go-host` admin/dashboard pages so they look like one coherent OS1 application from the first commit. It captures the lessons from the first dashboard visual-polish pass: importing the OS theme is not enough; every page must use the same shell model, spacing model, typography scale, semantic highlights, Storybook/MSW workflow, and screenshot review loop.

The target visual language is **classic Mac / OS1 admin console**: black outlines, striped title bars, compact bitmap-style typography, dense but readable tables, explicit panels, and careful use of subtle color highlights. The goal is not ornamental nostalgia. The goal is a legible operator UI where every control looks intentional and every page follows the same rules.
...
3. **What needs attention?** Warnings, dangerous options, unavailable features, and safe states use subtle semantic highlights.
...
- [ ] Are dangerous/unavailable concepts subtly highlighted?
...
7. Semantic highlights are used for warnings/safe/danger concepts.
