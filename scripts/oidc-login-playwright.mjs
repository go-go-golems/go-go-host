...
// Keycloak profile and Playwright installed. Gated so normal CI does not run it
// accidentally.
...
const baseURL = process.env.GO_GO_HOST_BASE_URL || 'http://127.0.0.1:5173';
...
try {
  ...
  await page.getByText(/platform admin/i).waitFor({ timeout: 30000 });
