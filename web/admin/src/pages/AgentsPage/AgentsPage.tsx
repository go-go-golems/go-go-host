...
import { useCreateAgentMutation, useListAgentKeysQuery, useListAgentsQuery, useRevokeAgentKeyMutation, useRevokeAgentMutation } from '../../services/goGoHostApi';
...
export function AgentsPage() {
  ...
  return <div className="agents-page">
    ...
    {selectedAgentId ? <section className="dashboard-panel"><h2>Signing keys</h2>{selectedKeys.isLoading ? <LoadingBlock lines={3} /> : selectedKeys.error ? <ErrorCallout title="Unable to load keys" error={apiErrorMessage(selectedKeys.error)} /> : <AgentKeysTable keys={selectedKeys.data ?? []} onRevoke={onRevokeKey} />}</section> : null}
  </div>;
}
