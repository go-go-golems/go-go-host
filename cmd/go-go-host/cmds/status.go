...
func NewStatusCommand() (*StatusCommand, error) {
	...
	return &StatusCommand{CommandDescription: glazedcmds.NewCommandDescription(
		"status",
		glazedcmds.WithShort("Check go-go-host daemon health"),
		glazedcmds.WithLong(`Check the go-go-host daemon health and version endpoints.

Examples:
  go-go-host status --api-url http://127.0.0.1:8080
  go-go-host status --output json
`),
